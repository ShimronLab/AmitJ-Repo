...
def recon_func(signal, Nx, Ny, Ny_acq):
    ...
    if len(acquired_lines) != n_ex * n_echo:
        raise ValueError(f"Mismatch: len(acquired_lines)={len(acquired_lines)} vs expected={n_ex * n_echo}")

    for idx in range(n_ex * n_echo):
        ky = acquired_lines[idx]
        ex = idx // n_echo
        e = idx % n_echo
        if 0 <= ky < Ny:
...
def generate_dataset(data_dir, seq_path, output_root, Nx=128, Ny=128, TE=96e-3, fov=200e-3,n_echo=64):
...
seq_path = os.path.expanduser("~/AmitProject/TSE_SEQ_CO_R2_TE96_TF128.seq")
output_root = os.path.expanduser("~/AmitProject")
generate_dataset(data_dir=data_dir, seq_path=seq_path, output_root=output_root)
